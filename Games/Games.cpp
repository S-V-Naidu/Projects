...
void chess();
bool isMoveValid(Chess::Position, Chess::Position, Chess::EnPassant*, Chess::Castling*, Chess::Promotion*);
void makeTheMove(Chess::Position, Chess::Position, Chess::EnPassant*, Chess::Castling*, Chess::Promotion*);
void newGame(void);
void undoMove(void);
void movePiece(void);
void saveGame(void);
...
{
	system("CLS");
	cout << "******Gaming zone*******" << endl;
	up: cout << "Select the game" << endl;
	cout << "1. Tic-Tac-Toe\n2. Chess\n3. Snake Arcade\n4. Exit" << endl;
	int choice;
	cin >> choice;
	switch (choice) {
...
void movePiece(void)
{
	...
	if (S_promotion.bApplied == true)
	{
		...
		to_record += '=';
		to_record += toupper(chPromoted); // always input with a capital letter
	}
	...
	if (true == current_game->playerKingInCheck())
	{
		...
		{
			// Add to the string with '+=' (e.g., piece captured)
			if (Chess::WHITE_PLAYER == current_game->getCurrentTurn())
			{
